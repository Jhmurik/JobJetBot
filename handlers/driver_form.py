...
@router.message(DriverForm.experience) async def process_experience(message: Message, state: FSMContext): await state.update_data(experience=message.text.strip()) await message.answer("🗣️ Какие языки вы знаете? (через запятую)") await state.set_state(DriverForm.languages)

▶️ Языки

@router.message(DriverForm.languages) async def process_languages(message: Message, state: FSMContext): raw = message.text.strip() languages = [lang.strip() for lang in raw.split(",") if lang.strip()] await state.update_data(languages=languages) await message.answer("📄 Какие у вас есть документы для работы?") await state.set_state(DriverForm.documents)
...
@router.message(DriverForm.ready_to_depart) async def process_ready_to_depart(message: Message, state: FSMContext): await state.update_data(ready_to_depart=message.text.strip()) await message.answer("📱 Ваши контактные данные (телефон, Telegram и т.д.):") await state.set_state(DriverForm.contacts)
...
@router.message(DriverForm.confirmation) async def process_confirmation(message: Message, state: FSMContext): if message.text.strip().lower() == "подтверждаю": data = await state.get_data() pool = message.bot.get("db")
